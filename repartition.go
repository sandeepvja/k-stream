...
func (r Repartition) Validate(s Side) error {
	fmt.Println(r)
	if r.StreamSide != s {
		return errors.New(`k-stream.stream_joiner.validation`, `stream side is not compatible`)
	}
	if r.KeyEncoder == nil {
		return errors.New(`k-stream.stream_joiner.validation`, `repartition key encoder can not be nil`)
	}
	if r.ValueEncoder == nil {
		return errors.New(`k-stream.stream_joiner.validation`, `repartition value encoder can not be nil`)
	}
	//if r.Topic.Name == `` {
	//	return errors.New(`k-stream.stream_joiner.validation`, `repartition topic can not be empty`)
	//}
