...
func (sj *SideJoiner) Run(ctx context.Context, kIn, vIn interface{}) (kOut, vOut interface{}, cont bool, err error) {
	...
	case `left`:
		...
		if err != nil {
			return nil, nil, false, errors.WithPrevious(err,
				`k-stream.joiner.side_joiner`, `value mapper failed`)
		}
	case `right`:
	...
	default:
		return nil, nil, false, errors.New(`k-stream.joiner.side_joiner`, `stream joiner sides should be only "left" and "right"`)
	}
