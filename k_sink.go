...
func (s *KSink) Build() (node.Node, error) {
	p, err := s.ProducerBuilder(&producer.Options{
		Id: producer.NewProducerId(s.topic(s.topic(s.TopicPrefix))),
	})
	if err != nil {
		return nil, errors.WithPrevious(err, `k-stream.KSink`, `cannot Build producer`)
	}
...
func WithProducer(p producer.Builder) SinkOption {
	return func(sink *KSink) {
		sink.ProducerBuilder = func(conf *producer.Options) (producer.Producer, error) {
			return p(nil)
		}
	}
}
