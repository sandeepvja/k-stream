...
func NewMockStore(name string, kEncode encoding.Encoder, vEncoder encoding.Encoder, backend store_backend.Backend) Store {
	return &MockStore{
		name:     name,
		vEncoder: vEncoder,
		kEncoder: kEncode,
...
func (s *MockStore) Backend() store_backend.Backend {
	return s.backend
}
