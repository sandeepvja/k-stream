...
func NewPool(id string, t *node.TopologyBuilder, metricsReporter metrics.Reporter, config *PoolConfig) *Pool {
	...
	p := &Pool{
		...
		order:    config.Order,
		workers:  make([]*worker, config.NumOfWorkers, config.NumOfWorkers),
		hasher:   fnv.New32a(),
		stopped:  make(chan bool, 1),
	}

	bufferUsage := metricsReporter.Counter(`k_stream_task_pool_worker_buffer`, []string{`pool_id`})

	for i := int64(config.NumOfWorkers) - 1; i >= 0; i-- {

		topology, err := t.Build()
		if err != nil {
			logger.DefaultLogger.Fatal(`k-stream.streamProcessor`, err)
		}
...
func (p *Pool) Run(ctx context.Context, key, val []byte, doneClb func()) {
	...
	w, err := p.worker(key)
	if err != nil {
		logger.DefaultLogger.ErrorContext(ctx, `k-stream.task_pool`, err)
		return
	}
	...
	case w.tasks <- t:
		break
	default:
		logger.DefaultLogger.Debug(`k-stream.task_pool`, `worker buffer full`)
		break
	}*/
}
...
func (w *worker) start() {
	...
	for task := range w.tasks {
		...
		if err != nil {
			logger.DefaultLogger.ErrorContext(task.ctx, `k-stream.task_pool`, err)
		}
		task.doneClb()
	}
}
